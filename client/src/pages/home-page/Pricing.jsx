...
const CheckIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-500 dark:text-green-400 mr-3 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
  </svg>;
const TimesIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-400 dark:text-gray-500 mr-3 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
...
const PricingCard = ({
  ...
  const cardClasses = `
    ...
    ${plan.isPopular ? 'border-2 border-indigo-500 dark:border-indigo-400 relative shadow-lg shadow-indigo-500/20 dark:shadow-indigo-400/20' : 'border-gray-200 dark:border-zinc-700 hover:border-gray-300 dark:hover:border-zinc-600'}
  `;
  const buttonClasses = `
    ...
    ${plan.isPopular ? 'bg-indigo-500 dark:bg-indigo-600 text-white shadow-lg shadow-indigo-500/30 dark:shadow-indigo-600/30 hover:bg-indigo-600 dark:hover:bg-indigo-700 focus:ring-indigo-500 dark:focus:ring-indigo-400' : `
  ...
  return <div className={cardClasses}>
      {plan.isPopular && <div className="absolute top-0 right-4 -mt-3 bg-indigo-500 dark:bg-indigo-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
          MOST POPULAR
        </div>}
      ...
      <p className="text-gray-800 dark:text-zinc-100 text-4xl font-bold mb-2">
        {plan.price}
        {plan.pricePeriod && <span className="text-lg text-gray-500 dark:text-zinc-400 font-medium">{plan.pricePeriod}</span>}
      </p>
      <p className="text-gray-600 dark:text-zinc-400 mb-8 text-sm h-10">{plan.description}</p>
      <ul className="space-y-4 mb-8">
        {plan.features.map((feature, index) => <li key={index} className={`flex items-center ${feature.included ? 'text-gray-700 dark:text-zinc-300' : 'text-gray-400 dark:text-zinc-500'}`}>
...
const Pricing = () => {
  return <section id="pricing" className="relative py-16 sm:py-24 overflow-hidden">
      {}
